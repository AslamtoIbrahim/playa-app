...
export default function Attendances({ attendances, sessions }: AttendancesIndexProps) {
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4 lg:p-8">
                ...
                <div className="flex flex-col justify-between gap-4 px-2 sm:flex-row sm:items-center">
                    ...
                    <AddAttendanceDialog sessions={sessions} />
                </div>
                ...
                <div className="flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <Table>
                        <TableHeader className="bg-slate-50/50">
                            <TableRow className="border-b border-slate-200 text-sm hover:bg-transparent">
                                ...
                                <TableHead className="font-bold text-slate-800">Date du Pointage</TableHead>
                                <TableHead className="font-bold text-slate-800 text-center">Journée</TableHead>
                                <TableHead className="text-right font-bold text-slate-800">Masse Salariale</TableHead>
                                    ...
                                    >
                                        ...
                                        <TableCell className="text-sm font-semibold text-slate-600">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {attendance.date ? format(new Date(attendance.date), 'dd MMMM yyyy') : '---'}
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell className="text-center">
                                            <div className="flex justify-center">
                                                {attendance.session ? (
                                                    ...
                                                    <span className="text-xs text-slate-400 italic">Sans session</span>
                                                )}
                                            </div>
                                        </TableCell>
                                        ...
                                        <TableCell className="bg-slate-50/30 text-right text-base font-black text-slate-900">
                                            {new Intl.NumberFormat('fr-FR', {
                                                style: 'currency',
                                                currency: 'MAD',
                                            }).format(Number(attendance.total_wage))}
                                        </TableCell>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50/50 px-6 py-4">
                        <div className="text-xs font-bold tracking-widest text-slate-500 uppercase">
                            {attendances.meta?.total || attendances.data.length} Feuilles au total
                        </div>
                            ...
                            {attendances.links?.map((link, i) => (
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        link.active && 'bg-slate-900 text-white scale-105',
                                        !link.url && 'opacity-30'
                                    )}
                                    asChild={!!link.url}
                                >
