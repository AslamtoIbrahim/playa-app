...
export function RowActions({ invoiceId, item, data }: RowActionsProps) {

    const handleAddRow = (direction: 'above' | 'below', duplicate: boolean = false) => {
        router.post(store(invoiceId), {
            ...(duplicate ? data : {
                boat_id: null,
                item_id: null,        // صيفط null أحسن من ""
                unit_count: 0,       // صيفط 0 باش يدوز الـ Validation numeric
                unit_price: 0,       // صيفط 0 باش يدوز الـ Validation numeric
                weight: 0,
                unit: "caisse"
            }),
            target_id: item.id,
